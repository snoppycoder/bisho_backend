...
membersRouter.post("/import", async (req, res) => {
  try {
      ...
      try {
        ...
        await prisma.memberBalance.upsert({
          ...
          update: {
            ...
            costOfShare: safeAmount(
              memberData["Credit Association Cost of Share"]
            ),
            registrationFee: safeAmount(
              memberData["Credit Association Registration Fee"]
            ),
            membershipFee: safeAmount(
              memberData["Credit Association Membership Fee"]
            ),
            willingDeposit: safeAmount(
    ...
    return res
      .status(500)
      .json({ error: "Failed to import members", details: error });
  }
});
...
membersRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const documentUrl = `/api/loans/documents/${Date.now()}-${
        file.originalname
      }`;
...
membersRouter.get("/:etNumber", async (req, res) => {
  ...
  const etNumber = Array.isArray(req.params.etNumber)
    ? req.params.etNumber[0]
    : req.params.etNumber;
  if (isNaN(etNumber)) {
